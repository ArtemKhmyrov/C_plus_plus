...
template <typename x>//заголовок
x **change(x **a, int n, int m)//переменные
{
	int b, c, v;//переменные
	b = 0;//приравниваем
	v = n - 1;//заменяем на нужную строчку
	c = n / 2;//идем до середины
	while (b != c)//цикл пока
	{
		for (int i = 0; i < m; i++)//цикл столбцов
		{
			swap(a[b][i], a[v][i]);	//свапаем
		}
		b++;//строка ++
		v--;// строка --
	}
...
int main()//оператор 3
{
	...
	do//оператор 4
	{
		...
		case 1://кейс 
		{
			...
			a = change(a, n, m);//вернуться к заданному оператору 
			cout << "New:\n";//вывод
			print(a, n, m);//вывод
			break;//завершение 
		}
		case 2://кейс 
		{
			...
			a1 = change(a1, n, m);//вернуться к заданному оператор
			cout << "New:\n";//вывод
			print(a1, n, m);//вывод
			break;//завершение 
		}
		case 3://кейс 
		{
			...
			a2 = change(a2, n, m);//вернуться к заданному оператор
			cout << "New:\n";//вывод
			print(a2, n, m);//вывод
			break;//завершение 
		...
		case 4://кейс 
		{
			...
			a3 = change(a3, n, m);//вернуться к заданному оператор
			cout << "New:\n";//вывод
			print(a3, n, m);//вывод
			break;//завершение 
		}
